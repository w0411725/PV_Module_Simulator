...
class Widget(QWidget):
    ...
    def generate_iv_curve(self):
        try:
            ...
            dark_saturation_current_A_m2 = float(self.ui.normalizedDarkSaturationCurrentInputField.text()) * 10  # A/m²
            normalized_jsc_A_m2 = float(self.ui.normalizedJscInputField.text()) * 10  # A/m²
            ...
            # Convert module dimensions to meters
            module_width = module_width_mm / 1000  # Convert mm to m
            module_length = module_length_mm / 1000  # Convert mm to m
            ...
            # Compute a more accurate V_oc estimate
            estimated_V_oc = ideality_factor * thermal_voltage * np.log1p(normalized_jsc_A_m2 / dark_saturation_current_A_m2)

            # Generate voltage values dynamically based on estimated V_oc
            voltage = np.linspace(0, 1.2 * estimated_V_oc, 2000)

            # Ensure J0 does not underflow (prevent numerical issues)
            min_J0 = 1e-3 * normalized_jsc_A_m2  # Keep J0 at least 0.1% of J_sc
            dark_saturation_current_A_m2 = max(dark_saturation_current_A_m2, min_J0)

            # --- Self-Consistent Calculation of Current Density ---
            exponent_term = voltage / (ideality_factor * thermal_voltage)
            exponent_term = np.clip(exponent_term, -50, 50)  # Prevent overflow
            diode_current = dark_saturation_current_A_m2 * (np.exp(exponent_term) - 1)

            # Correct for Rs
            voltage_effective = voltage - (intrinsic_series_resistance * normalized_jsc_A_m2)
            voltage_effective = np.maximum(voltage_effective, 0)  # Prevent negative voltages

            # Recalculate diode current based on Rs correction
            exponent_term = voltage_effective / (ideality_factor * thermal_voltage)
            exponent_term = np.clip(exponent_term, -50, 50)  # Prevent overflow
            diode_current = dark_saturation_current_A_m2 * (np.exp(exponent_term) - 1)

            # Final current density correction (Rs & Rsh applied)
            current_density = (normalized_jsc_A_m2 - diode_current) / (1 + (intrinsic_series_resistance / shunt_resistance))

            # Ensure numerical stability
            current_density = np.maximum(current_density, 0)
            ...
            # Determine maximum power point (Pmax)
            max_power_index = np.argmax(power)
            max_power = power[max_power_index]
            max_voltage = voltage[max_power_index]
            max_current = current[max_power_index]

            # Calculate incident power
            incident_power = light_intensity * area  # W

            # Calculate efficiency (PCE)
            efficiency = (max_power / incident_power) * 100

            # Generate J-V plot
            plt.figure(figsize=(6, 4))
            plt.plot(voltage, current, linestyle="solid", label="Final Corrected J-V Curve", color="blue")

            # Annotate Max Power Point
            ...
            print("Incident Power (W):", incident_power)
            print("Module Area (m²):", area)
            print("Voltage values:", voltage[:5])  # Print first few voltage values
            print("Current values:", current[:5])  # Print first few current values
            print(f"Max Power: {max_power:.2f} W")
            print(f"Efficiency (PCE): {efficiency:.2f} %")

        except ValueError as e:
            print(f"Error: Invalid input values. Ensure all fields are filled with numbers. Details: {e}")
